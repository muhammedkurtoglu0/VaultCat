...
### `ai_core/`
- `mcp_server.py` — FastMCP server (streamable HTTP, `127.0.0.1:8000`) exposing `run_privilege_escalation` and `run_secret_exfiltration` as MCP tools. Maintains a session-level `pentest_context` dict to pass captured tokens between tool calls.
- `chat_ui.py` — interactive loop that connects to a local Ollama model and the MCP server, bridging natural-language operator input to MCP tool calls.
- `helper.py` — shared utilities for the AI layer.
